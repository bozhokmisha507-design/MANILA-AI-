...
from handlers.styles import (
    ...
    model_selected_cb,   # если есть выбор моделей
...
from handlers.single import single_command, single_style_callback   # одиночное GPT
...
async def main_async():
    ...
    try:
        ...
        # ---------- ОЧИСТКА БД от записей о несуществующих файлах (Вариант 3) ----------
    ...
    if 'model_selected_cb' in locals():
    ...
    application.add_handler(CallbackQueryHandler(single_style_callback, pattern="^single_style_"))
    ...
    # Кнопки главного меню (включая новую кнопку для одиночного GPT)
    application.add_handler(MessageHandler(
        filters.Text(["📤 Загрузить фото", "🖼️ Стили", "❓ Помощь", "🗑 Очистить селфи", "🏠 Главное меню", "💎 Мои жетоны", "✨ Одно фото (GPT Max)"]),
