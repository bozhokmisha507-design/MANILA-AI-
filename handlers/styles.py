...
async def show_styles_menu(target, context=None):
    ...
    text = (
        "🎨 *Выбери сценарий для твоей фотосессии из 8 кадров:*\n\n"
        "• Интерьерная съёмка\n• На лавочке в парке\n• Набережная\n"
        "• Кафе / ресторан\n• Минималистичная студия\n• Городской сквер\n\n"
        "Каждый стиль даст тебе 8 уникальных фото в разных ракурсах!"
    )

    if isinstance(target, int):
        if context is None:
            return
        await context.bot.send_message(chat_id=target, text=text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
    else:
...
async def style_selected_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if photo_count < Config.MIN_PHOTOS:
        await query.edit_message_text(f"⚠️ Нужно минимум {Config.MIN_PHOTOS} фото. Сейчас: {photo_count}\n\nНажми «📤 Загрузить фото».")
        return
    ...
    # Показать выбор модели (пакет 8 фото)
    keyboard = []
    ...
    await query.edit_message_text(
        f"✅ Стиль *{style['name']}* выбран.\n\nТеперь выбери модель генерации (пакет из 8 фото):",
        parse_mode='Markdown',
...
async def model_selected_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"Пакет из 8 фотографий стоит {model_info['price_rub']}₽ или {model_info['price_tokens']} жетонов.\n\n"
        "👇 Как хочешь оплатить?",
        parse_mode='Markdown',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
# ------------------------------------------------------------
#  Оплата жетонами (вызов генерации)
# ------------------------------------------------------------
async def pay_with_tokens_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await db.use_tokens(user_id, model_info['price_tokens']):
        await query.edit_message_text("❌ Недостаточно жетонов. Пополни баланс.")
        return
...
# ------------------------------------------------------------
#  Оплата деньгами – создание заказа в ЮKassa
# ------------------------------------------------------------
async def pay_with_money_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text("❌ Не удалось создать ссылку на оплату. Попробуйте позже.")
        return
...
# ------------------------------------------------------------
#  Генерация пакета (для оплаты жетонами)
# ------------------------------------------------------------
async def generate_package(user_id: int, bot: Bot, db, context, style_key: str, model_key: str):
    try:
        ...
        logger.error(f"Ошибка генерации пакета: {e}", exc_info=True)
        await bot.send_message(user_id, "❌ Произошла ошибка. Мы уже работаем над исправлением.")
        token_cost = Config.PACKAGE_MODELS.get(model_key, {}).get('price_tokens', 4)
        await db.add_tokens(user_id, token_cost)
        await bot.send_message(user_id, f"💎 Жетоны в количестве {token_cost} возвращены.")

# ------------------------------------------------------------
#  Генерация пакета из вебхука (после оплаты деньгами)
# ------------------------------------------------------------
async def generate_package_from_data(user_id: int, bot: Bot, db, data: dict):
    try:
        ...
        if not style_key:
            logger.error(f"Нет style_key в данных заказа: {data}")
        ...
        logger.error(f"Ошибка генерации пакета из данных: {e}", exc_info=True)
        await bot.send_message(user_id, "❌ Произошла ошибка. Мы уже работаем над исправлением.")
        raise

# ------------------------------------------------------------
#  Экспорт обработчиков (для bot.py)
# ------------------------------------------------------------
